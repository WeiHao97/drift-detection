...
def main(args):
    ...
    data_loader = torch.utils.data.DataLoader(
        dataset, batch_sampler=train_batch_sampler, num_workers=args.workers, collate_fn=util.collate_fn
    )

    data_loader_test = torch.utils.data.DataLoader(
        dataset_test, batch_size=1, sampler=test_sampler, num_workers=args.workers, collate_fn=util.collate_fn
    )
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir:
            ...
            util.save_on_master(checkpoint, os.path.join(args.output_dir, f"model_{epoch}.pth"))
            util.save_on_master(checkpoint, os.path.join(args.output_dir, "checkpoint.pth"))
